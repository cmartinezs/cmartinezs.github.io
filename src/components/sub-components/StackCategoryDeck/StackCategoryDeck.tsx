...
export function StackCategoryDeck() {
  ...
  useEffect(() => {
    ...
    const cards = viewport.querySelectorAll<HTMLElement>(".stack-category-card");
    const card = cards[activeIndex];
    if (!card) return;
    const vpW = viewport.clientWidth;
    const cardW = card.offsetWidth;
    viewport.scrollTo({
      left: card.offsetLeft - (vpW - cardW) / 2,
      behavior: "smooth",
    });
  }, [activeIndex, isMobile]);

  return (
    <div className="stack-categories-viewport" ref={viewportRef}>
        ...
        {stackCategories.map((cat, i) => {
          ...
          const desktopStyle = isMobile ? {} : {
            ...
            filter: isActive
              ...
              : `saturate(${Math.max(0.5, 0.84 - distance * 0.1)}) brightness(${Math.max(0.6, 0.85 - distance * 0.1)})`,
