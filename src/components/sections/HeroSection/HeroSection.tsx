...
import { staggerContainer, staggerFadeUp, floatAnimation } from "@/lib/animations";
...
export function HeroSection() {
  ...
  const { eyebrow, title, tagline, actions } = heroContent;
  ...
  const content = (
    <div className="container relative z-10">
      <div className="grid lg:grid-cols-2 gap-12 items-center">
        ...
        {shouldAnimate ? (
          <motion.div variants={staggerFadeUp} className="hero-side-shell">
            <HeroSide shouldAnimate />
          </motion.div>
        ) : (
          <div className="hero-side-shell">
            <HeroSide shouldAnimate={false} />
          </div>
        )}
      </div>
    </div>
...
function HeroSide({ shouldAnimate }: { shouldAnimate: boolean }) {
  const { lead, signals } = heroContent;
